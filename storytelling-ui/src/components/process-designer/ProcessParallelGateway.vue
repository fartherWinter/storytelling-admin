<template>
  <div 
    :class="['process-node', 'parallel-gateway', { selected: isSelected, preview: isPreview }]"
    :style="nodeStyle"
    @click="handleClick"
  ...
  >
    ...
    <!-- 节点标签 -->
    <div class="node-label">{{ nodeData.label || '并行网关' }}</div>
    ...
    <div v-if="!isPreview" class="connection-points">
      ...
      ></div>
      <div 
        class="connection-point output-right"
        @click.stop="handleConnectionClick('output')"
        @mousedown.stop
      ></div>
    </div>
...
.parallel-gateway:hover {
  transform: scale(1.05);
}

.parallel-gateway.selected {
  filter: drop-shadow(0 0 8px rgba(103, 194, 58, 0.6));
}

.parallel-gateway.preview {
  cursor: default;
  pointer-events: none;
}
...
.parallel-gateway:hover .diamond-shape {
  box-shadow: 0 4px 12px rgba(103, 194, 58, 0.4);
}

.parallel-gateway.selected .diamond-shape {
  border-color: #409eff;
}
...
.connection-point.output-right:hover {
