...
public enum ResultCode {
     ...
     * 当前开发者不存在:身份校验失败
     */
    IDENTITY_NOT_EXIST(5000, "开发者身份不存在。"),
    /**
     * 数据签名校验失败
     */
    SIGN_FAILED(5001, "sign签名校验失败。"),
    /**
     * 黑名单
     */
    BLACK_VERIFY(5002, "用户尚未启用"),
    /**
     * 用户资料完善
     */
    MOVIE_USER_PERFECT(5003, "用户资料未完善，请完善"),
    /**
     * 用户未进行实名认证
     */
    REAL_NAME_FAIL(5004, "用户未进行实名认证，请进行实名认证"),
    /**
     * 当前用户累计实名次数超过限制
     */
    REAL_NAME_COUNT(5005, "用户累计实名次数超过限制，请联系客服"),
    /**
     * 用户不存在，请先注册后加入白名单
     */
    USER_NOT_EXIST_WHITE_LIST(5006, "用户不存在，请先注册后再加入白名单"),
    /**
     * 用户处于黑名单
     */
    USER_BLACK_BLOCK(5007, "账号异常，请联系客服处理"),
    /**
     * 请求时间戳已过期
     */
    TIMESTAMP_EXPIRE(5008, "请求时间戳已过期"),
    /**
     * 限流
     */
    REQUEST_LIMIT(6001, "当前时间段访问数量过多，请稍后重试"),
    /**
     * 限座
     */
    SEAT_LIMIT(7001, "不能重复购买同一座位"),
    /**
     * 限流
     */
    FILM_LIMIT(7002, "暂无可用影片"),
    /**
     * 订单数量限制
     */
    REQUEST_ORDER_LIMIT(6002, "当前网络拥堵，服务器已收到您的请求，排队处理中，请稍后再试。"),
    /**
     * 系统异常
     */
     ...
     * 用户
     */
    USER_ADMIN_ERROR(20000, "初始账号无法被删除"),
    ...
    ORDER_INSUFFICIENT_PERMISSIONS(31017, "你没有权限获取该订单信息"),
    ORDER_CANCELLATION_ERROR(31004, "订单已支付，无法取消订单"),
    ORDER_UNSHIPPED(31005, "订单未发货，无法确认收货"),
    ORDER_INCOMPLETE_NOTCLOSED(31006, "订单未完成或未关闭，无法删除订单"),
    ORDER_REMOVE_SUCCESS(31007, "订单删除成功"),
    ...
    INSUFFICIENT_BALANCE_OF_CARD(32014, "卡券余额不足"),
