...
系统已集成ShardingSphere，支持商城模块的分库分表：

### 分片策略
- **mall_user**: 基于用户ID进行分片
- **mall_order**: 基于订单ID进行分片
- **mall_sub_order**: 基于子订单ID进行分片
  ...
  shardingsphere:
    ...
    rules:
      sharding:
        tables:
          mall_user:
            actual-data-nodes: ds$->{0..1}.mall_user_$->{0..1}
            database-strategy:
              standard:
                sharding-column: id
...
**注意**: 本项目仅供学习和参考使用，生产环境部署请根据实际情况调整配置。